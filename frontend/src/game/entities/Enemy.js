...
export class Enemy extends Phaser.Physics.Arcade.Sprite {
  constructor(scene, x, y, texture, statsConfig) {
    ...
    if (this.tipoEnemigo === 'KAMIKAZE') {
      this.luzPersonal = scene.lights.addLight(x, y, 120, 0xff0000, 0.5);
    } else {
      this.luzPersonal = scene.lights.addLight(x, y, 220, 0xfffdeb, 0.5);
    }
  }
  ...
  desactivar() {
    ...
    if (this.luzPersonal) {
      this.scene.lights.removeLight(this.luzPersonal);
